...
// 4. CONSULTA DE TODOS LOS PRODUCTOS
...
router.get('/historial', async (req, res) => {
  try {
    const query = `
      ...
      JOIN gestion_comercial.dim_producto p 
        ON m.producto_id = p.producto_id
      JOIN gestion_comercial.dim_tipo_movimiento t 
        ON m.tipo_movimiento_id = t.tipo_movimiento_id
...
router.post('/finalizar-venta', async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(productos) || productos.length === 0) {
      throw new Error("No se recibieron productos para vender");
    }
    ...
    // 2. Buscar el ID REAL de salida_venta
    ...
    if (idSalidaRes.rows.length === 0) {
      throw new Error("No existe el tipo de movimiento 'salida_venta'");
    }

    const idSalida = idSalidaRes.rows[0].tipo_movimiento_id;

    // 3. Insertar cada producto vendido DESCONTANDO DEL ESTANTE
    for (const item of productos) {
      const cantidadVendida = Number(item.cantidad || item.quantity || 1);
      const precioItem = Number(item.precio || item.precio_unitario || 0);

      if (!item.producto_id) {
        throw new Error("Hay un producto sin producto_id");
      }
    ...
    console.error("❌ Error al finalizar venta:", err.message);
    res.status(500).json({ error: err.message });
  } finally {
    client.release();
  }
